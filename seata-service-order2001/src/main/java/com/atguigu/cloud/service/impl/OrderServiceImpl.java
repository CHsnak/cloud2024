...
public class OrderServiceImpl implements OrderService {
    ...
    public void create(Order order) {
        ...
        if (i > 0) {
            log.info("日志 ----- 订单创建成功 ----- Order = " + orderFromDB);
            orderFromDB = orderMapper.selectOne(order);

            log.info("日志 ----- 调用库存微服务 -- StorageFeignApi -- 减少库存数目");
            // 2.调用storage微服务，更新库存，减少商品数目
            storageFeignApi.decrease(orderFromDB.getProductId(), orderFromDB.getCount());
            log.info("日志 ----- 调用库存微服务 -- StorageFeignApi -- 成功！");

            log.info("日志 ----- 调用账户微服务 -- AccountFeignApi -- 减少账户余额");
            // 3.调用account微服务，扣款，减少用户余额
            accountFeignApi.decrease(orderFromDB.getUserId(), orderFromDB.getMoney());
            log.info("日志 ----- 调用账户微服务 -- AccountFeignApi -- 成功！");

            log.info("日志 ----- 修改订单状态 ----- 修改中...");
            // 4.订单状态更新，改为1（已完成）
            orderFromDB.setStatus(1);
            orderMapper.updateByPrimaryKey(orderFromDB);
            log.info("日志 ----- 修改订单状态 ----- 已完成！");
        }
        log.info("日志 ----- 订单创建全部完成 ----- XID = " + xid);
    }
}
